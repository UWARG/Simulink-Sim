...
num_ac = 3;  % number of aircraft in collaborative control
num_instincts = 4;  % number of instincts that aircraft are responding to

% Aircraft collision avoidance variables
minimum_ac_dist = 30; % minimum separation distance between aircraft
...
%                        300];   
obstacles.positions = [3000        -20       10500
                       4500         30       10000];
obstacles.radius    = [10;
                       15];   

minimum_obs_dist = 30; % minimum separation distance between aircraft and obstacles
sigma_o = 100; % decay rate in ft for collision avoidance
%desired_vel=[0 400 0]; % desired velocity vector for formation

% Target acquisition/flyby variables
targets.tgt_acquire_on = 0; % turn target acquisition on/off
desired_vel=[400 0 0]; % desired velocity vector for formation
sigma_t1 = 50;   % decay rate in ft/s for desired heading and velocity acquisition
sigma_t2 = 1000; % decay rate in ft/s for target acquisition (vel and direction acquisition)
% original set
...
state_ic.deltay_ic = 30; % will stagger this distance to the left and right of lead
